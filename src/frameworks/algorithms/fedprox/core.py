...
class Client(BaseClient): 
    ...
    def train(
        ...
        Args:
            ...
            mu_prox (float): Proximity term for local regularization.
        ...
        # Save a deepcopy of the global model for FedProx proximal term
        global_params = None
        if model_state_dict is not None:
        ...
        for _ in range(num_epochs): 
            for inputs, targets in self.train_dataloader: 
                ...
                if global_params is not None and mu_prox > 0:
                    prox_term = 0.0
                    for (name, param) in self.model.named_parameters():
                        if name in global_params and param.requires_grad:
                            prox_term = prox_term + (param - global_params[name]).pow(2).sum()
                    loss = loss + (mu_prox / 2.0) * prox_term
        ...
        del criterion, optimizer, global_params
